...
そこで、同パッケージのベータAPIである`PrefectDbtOrchestrator`(`prefect_dbt.core._orchestrator`、パッケージの公開APIからはexportされていない内部モジュール)に切り替えました。こちらは`manifest.json`を自前でパースしてノードの実行順序(wave)を計算し、`PER_NODE`モードでは各ノードを本当に独立したPrefect Task/プロセスとして実行します。
...
`PER_NODE`にすることで、Prefect UI上でモデルごとに独立したTask run(成功/失敗/実行時間)が確認できるようになり、後述のノード単位キャッシュも有効化できるようになりました。ベータAPIなので非公開モジュールからのimportが必要な点だけ注意が必要です。
...
CI用のdeploymentをPrefect側に作ることも考えましたが、CIは「PRごとに1回、短命に、PRチェックとして結果を返す」用途であり、Prefectのスケジューリングや観測性(per-nodeタスク可視化)は不要です。そのため**CIはPrefectを経由せず、dbt-core単体でGitHub Actions上で完結**させました。
...
原因は、Prefect Cloudのmanaged実行環境(`pip_packages`)でdbt-snowflakeのバージョンを固定していなかったことでした。`pip_packages`はバージョン指定なしだと毎回pipが最新版を解決するため、`uv.lock`で固定しているローカル/CIの環境と静かにバージョンがズレてしまいます。
